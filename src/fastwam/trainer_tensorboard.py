...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        self.save_steps = self._parse_save_steps(cfg.get("save_steps", []))
        ...
        self.train_flow_expert_only = bool(cfg.get("train_flow_expert_only", False))
        ...
        expected_world_size = cfg.get("expected_world_size", None)
        if expected_world_size is not None:
            expected_world_size = int(expected_world_size)
            actual_world_size = int(self.accelerator.num_processes)
            if actual_world_size != expected_world_size:
                raise RuntimeError(
                    f"Expected world_size={expected_world_size}, got {actual_world_size}. "
                    "Check NNODES/FASTWAM_NUM_MACHINES/NPROC_PER_NODE and the DLC worker count."
                )
        ...
        flow_expert = getattr(self.model, "flow_expert", None)
        if self.train_flow_expert_only:
            if flow_expert is None:
                raise ValueError("`train_flow_expert_only=true` requires model.flow_expert.")
            setattr(self.model, "flow_only_training", True)
            self._apply_flow_only_train_mode(self.model)
            trainable_params = list(flow_expert.parameters())
            logger.info("Training flow_expert only for this run.")
        else:
            ...
            if flow_expert is not None:
                trainable_params.extend(list(flow_expert.parameters()))
        trainable_params = self._deduplicate_params(trainable_params)
    ...
    def _parse_save_steps(save_steps) -> set[int]:
        if save_steps in (None, "", "null"):
            return set()
        if isinstance(save_steps, str):
            raw_steps = [item.strip() for item in save_steps.split(",") if item.strip()]
        else:
            raw_steps = list(save_steps)
        steps = {int(step) for step in raw_steps}
        invalid_steps = sorted(step for step in steps if step <= 0)
        if invalid_steps:
            raise ValueError(f"`save_steps` must contain positive step ids, got {invalid_steps}")
        return steps

    def _should_save_checkpoint(self) -> bool:
        if self.global_step in self.save_steps:
            return True
        return self.save_every > 0 and self.global_step % self.save_every == 0
    ...
    def _set_dit_only_train_mode(self):
        ...
        if self.train_flow_expert_only:
            logger.info("Setting flow_expert to train mode and freezing other model components.")
            setattr(model, "flow_only_training", True)
            self._apply_flow_only_train_mode(model)
        else:
    ...
    def _apply_dit_only_train_mode(model):
        ...
        flow_expert = getattr(model, "flow_expert", None)
        if flow_expert is not None:
            flow_expert.train()
            flow_expert.requires_grad_(True)

    @staticmethod
    def _apply_flow_only_train_mode(model):
        model.eval()
        model.requires_grad_(False)
        flow_expert = getattr(model, "flow_expert", None)
        if flow_expert is None:
            raise ValueError("Flow-only training requested but model has no flow_expert.")
        flow_expert.train()
        flow_expert.requires_grad_(True)

    @staticmethod
    def _deduplicate_params(params):
        unique = []
        seen = set()
        for param in params:
            param_id = id(param)
    ...
    def _to_batched_eval_sample(sample):
        ...
        flow_gt = sample.get("flow_gt", None)
        flow_mask = sample.get("flow_mask", None)
        ...
        if flow_gt is not None:
            if not isinstance(flow_gt, torch.Tensor):
                raise TypeError(f"`sample['flow_gt']` must be a torch.Tensor, got {type(flow_gt)}")
            if flow_gt.ndim == 3:
                flow_gt = flow_gt.unsqueeze(0)
            if flow_gt.ndim != 4:
                raise ValueError(f"`sample['flow_gt']` must be 4D [B,2,H,W], got shape {tuple(flow_gt.shape)}")
            batched["flow_gt"] = flow_gt
        if flow_mask is not None:
            if not isinstance(flow_mask, torch.Tensor):
                raise TypeError(f"`sample['flow_mask']` must be a torch.Tensor, got {type(flow_mask)}")
            if flow_mask.ndim == 2:
                flow_mask = flow_mask.unsqueeze(0)
            if flow_mask.ndim not in (3, 4):
                raise ValueError(f"`sample['flow_mask']` must be [B,H,W] or [B,1,H,W], got shape {tuple(flow_mask.shape)}")
            batched["flow_mask"] = flow_mask
        return batched
                ...
                with self.accelerator.accumulate(self.model):
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        if self.global_step >= self.max_steps:
                            ckpt_info = None
                            if self.global_step not in self.save_steps:
                                ckpt_info = self.save_checkpoint()
                            if self.accelerator.is_main_process:
                                logger.info(
                                    "[done] max_steps reached step=%d weights=%s state=%s",
                                    self.global_step,
                                    None if ckpt_info is None else ckpt_info["weights_path"],
                                    None if ckpt_info is None else ckpt_info["state_path"],
                                )
                            return

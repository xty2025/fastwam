...
class WanVideoDiT(torch.nn.Module):
    ...
    def patchify(self, x: torch.Tensor, control_camera_latents_input: Optional[torch.Tensor] = None):
        ...
        try:
            ...
            message = str(exc)
            if "GET was unable to find an engine" not in message and "Misaligned Operand" not in message:
                raise
            logger.warning(
                "Conv3d patch_embedding hit backend alignment issue; retrying with cloned contiguous tensors. "
                "shape=%s stride=%s dtype=%s device=%s weight_shape=%s weight_stride=%s weight_dtype=%s",
                tuple(x.shape),
                tuple(x.stride()),
                x.dtype,
                x.device,
                tuple(self.patch_embedding.weight.shape),
                tuple(self.patch_embedding.weight.stride()),
                self.patch_embedding.weight.dtype,
            )
            weight = self.patch_embedding.weight.contiguous().clone()
    ...
    def pre_dit(
        ...
        batch_size = x.shape[0]     ## [B, C, T, H, W]
        ...
        if self.seperated_timestep:
            ...
            ) * timestep.view(batch_size, 1, 1)     # [B, T, tokens_per_frame]
            if fuse_vae_embedding_in_latents:
                # first frame use timestep 0, which will be the same as the timestep used for vae embedding if fuse_vae_embedding_in_latents is True, so that the model can learn to align the first latent frame with the vae embedding when timestep=0
                token_timesteps[:, 0, :] = 0
            token_timesteps = token_timesteps.reshape(batch_size, -1)   # [B, T*tokens_per_frame]
            token_t_emb = sinusoidal_embedding_1d(self.freq_dim, token_timesteps.reshape(-1))   #[B* T*tokens_per_frame, freq_dim]
            t = self.time_embedding(token_t_emb).reshape(batch_size, -1, self.hidden_dim)   #[B, T*tokens_per_frame, hidden_dim]
            t_mod = self.time_projection(t).unflatten(2, (6, self.hidden_dim))  # [B, T*tokens_per_frame, 6, hidden_dim]
        else:
        ...
        x = self.patchify(x, control_camera_latents_input=control_camera_latents_input) # [B, hidden_dim, T, H//patch_h, W//patch_w]
        f, h, w = x.shape[2:]

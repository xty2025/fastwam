...
class FastWAM(torch.nn.Module):
    ...
    def save_checkpoint(self, path, optimizer=None, step=None):
        ...
        if getattr(self, "flow_expert", None) is not None:
            payload["flow_expert"] = self.flow_expert.state_dict()
        if self.proprio_encoder is not None:
    ...
    def load_checkpoint(self, path, optimizer=None):
        ...
        if getattr(self, "flow_expert", None) is not None:
            if "flow_expert" in payload:
                self.flow_expert.load_state_dict(payload["flow_expert"], strict=False)
            else:
                logger.warning("Checkpoint has no `flow_expert` weights; keeping current `flow_expert` params.")
        elif "flow_expert" in payload:
            logger.warning("Checkpoint contains `flow_expert` weights but current model has no flow_expert; ignoring.")
        if self.proprio_encoder is not None:
